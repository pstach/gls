...
static code_t bytecoder_literal_to_code(const literal_t l)
{
...
static void bytecoder_output(const code_t c, bc_state_t *state)
{
	assert (c < BC_MAXCODE);
	if (state->buffull == state->bufalloc)
    {
		...
		state->bufalloc *= 2;
		code_t *newbuf = realloc(state->buffer, state->bufalloc * sizeof(code_t));
		if(newbuf == NULL)
			abort();
		state->buffer = newbuf;
    }
...
static size_t dict_matchlen(const bc_state_t *state, const int entry)
{
	size_t i;
...
static int dict_longestmatch(size_t *len, const bc_state_t *state, const int partial)
{
	int i, matchidx = -1;
	size_t t, matchlen = 0;
	const bc_dict_t *dict = state->dict;
...
static void coder_histadd(const literal_t c, bc_state_t *state)
{
...
static void coder_histremove(const size_t n, bc_state_t *state)
{
	size_t i;
	if (n > state->nrstored)
		abort();
 ...
 */
static void coder_outputbest(bc_state_t *state)
{
	int best;
	size_t bestlen;
	const bc_dict_t *dict = state->dict;
	...
	if (best != -1)
	{
		/* Dictionary code 0 is special and means: no output */
		if(dict->code[best] != 0)
...
bc_state_t *bytecoder_init(const bc_dict_t *dict)
{
	int i;
	bc_state_t *state;
...
size_t bytecoder_size(const bc_state_t *state)
{
...
void bytecoder_read(code_t *dst, bc_state_t *state)
{
	memmove(dst, state->buffer, state->buffull * sizeof (code_t));
	state->buffull = 0;
}

void bytecoder_clear(bc_state_t *state)
{
	...
	if(state->buffer)
		free(state->buffer);
	memset(state, 0, sizeof(bc_state_t));
	free(state);
	return;
}


void bytecoder(const literal_t c, bc_state_t *state)
{
	...
	if(state->dict != NULL)
	{
		...
		/* If all history[] matches some dictionary entry (possibly partially), we do nothing
 ...
 */
static double prac_chain(const unsigned long n, const double v, const double addcost,
...
double prac_best(double *mul, const unsigned long n, const int m_parm,
...
double prac_bytecode(const unsigned long k, const double addcost,

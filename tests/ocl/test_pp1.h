...
int pp1_stage1_mpz(mpz_t f, mpz_t X, modmpz_t m, pp1_plan_t *plan);
int pp1_stage1_ul64(ul64 f, ul64 X, mod64 m, pp1_plan_t *plan);
int pp1_stage1_ul128(ul128 f, ul128 X, mod128 m, pp1_plan_t *plan);

void pp1_stage2_mpz(mpz_t f, mpz_t X, modmpz_t m, stage2_plan_t *plan);
void pp1_stage2_ul64(ul64 f, ul64 X, mod64 m, stage2_plan_t *plan);
void pp1_stage2_ul128(ul128 f, ul128 X, mod128 m, stage2_plan_t *plan);




int ul_pp1op_test(struct state_t *state) {
	int ret = 0;
	cl_int err = 0;
	...
	ret = ul_test_setup(state, ul_pp1_stage1, &test_state_s1, global_work_size);
	if (ret)
		goto CLEANUP;
	...
	ocl_mem(dev_plan, ocl_pp1_plan_t, state->cxGPUContext, CL_MEM_READ_ONLY, 1);
	ocl_mem(dev_plan_bc, u_int8_t, state->cxGPUContext, CL_MEM_READ_ONLY, g_config.pp1_stage1_plan.bc_len);
	ocl_mem(dev_bt, cl_int, state->cxGPUContext, CL_MEM_WRITE_ONLY, global_work_size);
	ocl_mem(_dev_bt, cl_int, state->cxGPUContext, CL_MEM_WRITE_ONLY, global_work_size);
	ocl_mem(dev_stage2_plan, ocl_stage2_plan_t, state->cxGPUContext, CL_MEM_READ_ONLY, 1);
	ocl_mem(dev_S1, u_int32_t, state->cxGPUContext, CL_MEM_READ_ONLY, g_config.pp1_stage1_plan.stage2.n_S1);
	ocl_mem(dev_pairs, u_int8_t, state->cxGPUContext, CL_MEM_READ_ONLY, g_config.pp1_stage1_plan.stage2.n_pairs);
	...
	heap_mem(hst_plan, ocl_pp1_plan_t, 1);
	heap_mem(hst_bt, cl_int, global_work_size);
	heap_mem(hst_stage2_plan, ocl_stage2_plan_t, 1);

	/* Copy the plan to the device */
	{
		hst_plan->B1 = g_config.pp1_stage1_plan.B1;
		hst_plan->bc_len = g_config.pp1_stage1_plan.bc_len;
		hst_plan->exp2 = g_config.pp1_stage1_plan.exp2;

		hst_stage2_plan->B2 = g_config.pp1_stage1_plan.stage2.B2;
		hst_stage2_plan->d = g_config.pp1_stage1_plan.stage2.d;
		hst_stage2_plan->i0 = g_config.pp1_stage1_plan.stage2.i0;
		hst_stage2_plan->i1 = g_config.pp1_stage1_plan.stage2.i1;
		hst_stage2_plan->n_S1 = g_config.pp1_stage1_plan.stage2.n_S1;

		err = clEnqueueWriteBuffer(state->cqCommandQueue,
				dev_plan, CL_TRUE, 0,
				sizeof(ocl_pp1_plan_t),
				hst_plan, 0, NULL, NULL);
		err |= clEnqueueWriteBuffer(state->cqCommandQueue,
				dev_plan_bc, CL_TRUE, 0,
				sizeof(u_int8_t) * g_config.pp1_stage1_plan.bc_len,
				g_config.pp1_stage1_plan.bc, 0, NULL, NULL);
		err |= clEnqueueWriteBuffer(state->cqCommandQueue,
		...
		err |= clEnqueueWriteBuffer(state->cqCommandQueue,
				dev_S1, CL_TRUE, 0,
				sizeof(u_int32_t) * g_config.pp1_stage1_plan.stage2.n_S1,
				g_config.pp1_stage1_plan.stage2.S1, 0, NULL, NULL);
		err |= clEnqueueWriteBuffer(state->cqCommandQueue,
				dev_pairs, CL_TRUE, 0,
				sizeof(u_int8_t) * g_config.pp1_stage1_plan.stage2.n_pairs,
				g_config.pp1_stage1_plan.stage2.pairs, 0, NULL, NULL);
		if (CL_SUCCESS != err)
	...
	/* Analyze */
	{
		...
		for (size_t i = 0; i < global_work_size; i++) {
			...
			case 64:
				{
					...
					ul64_set_ui(ul_f, 0);
					ul64_set_ui(ul_X, 0);
					gmp_bt = pp1_stage1_ul64(ul_f, ul_X, ul_mod, &g_config.pp1_stage1_plan);
					if(ul64_cmp_ui(ul_f, 1) != 0) {
					}
					else {
						pp1_stage2_ul64(ul_f, ul_X, ul_mod, &g_config.pp1_stage1_plan.stage2);
					}
			...
			case 128:
				{
					...
					ul128_set_ui(ul_f, 0);
					ul128_set_ui(ul_X, 0);
					gmp_bt = pp1_stage1_ul128(ul_f, ul_X, ul_mod, &g_config.pp1_stage1_plan);
					if(ul128_cmp_ui(ul_f, 1) != 0) {
					}
					else {
						pp1_stage2_ul128(ul_f, ul_X, ul_mod, &g_config.pp1_stage1_plan.stage2);
					}
			...
			default:
				mpz_set_ui(gmpf, 0);
				mpz_set_ui(gmpX, 0);
				gmp_bt = pp1_stage1_mpz(gmpf, gmpX, mod, &g_config.pp1_stage1_plan);
				if(mpz_cmp_ui(gmpf, 1) != 0) {
				}
				else {
					pp1_stage2_mpz(gmpf, gmpX, mod, &g_config.pp1_stage1_plan.stage2);
				}
			}
